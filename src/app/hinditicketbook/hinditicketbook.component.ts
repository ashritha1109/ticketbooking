...
@Component({
  selector: 'app-hinditicketbook',
  templateUrl: './hinditicketbook.component.html',
  styleUrls: ['./hinditicketbook.component.css']
})
export class HinditicketbookComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.service.getSingleHindiMovieData(this.id).subscribe((data) => {
      console.log(data);
      this.ticketDetails.controls.name.setValue(data.name);
      this.ticketDetails.controls.lang.setValue('Hindi');
      ...
      if (tickets != null) {
        if (this.movieDetails.morning_show < tickets) {
          ...
          if (tickets != null) {
            ...
            this.service
              .editHindiTicketsData({ ...this.movieDetails, id: this.id })
              .subscribe((data) => {
                alert('edited');
